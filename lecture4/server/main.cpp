...
bool InernalSendMessage(string from, string to, string channel, string text, string &error) {
	error = "";
...
bool SendChatMessage(string from, string to, string text, string &error) {
	...
	if (to.rfind("@", 0) != 0) {
		...
		return InernalSendMessage(from, to, "", text, error);
	}
	...
	for (string& usrName : v) {
		...
		InernalSendMessage(from, usrName, to, text, t);
	}
	return true;
}
...
void HandleClientState(ConnectionData *conData, IoOperationData *pIoData) {
	...
	case PacketTypeEnum::CHANNELS:
		...
		status = "OK";
		res = vector_to_string(GetChannelsList());
		break;
